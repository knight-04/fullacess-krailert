...
const Sidebar = ({ isOpen, onClose, onLogout, activeTab, onTabChange }) => {
    ...
    const MenuItem = ({ text, icon: Icon, onClick, isActive, hasSubmenu, isExpanded }) => (
        <button
            onClick={onClick}
            className={`flex items-center justify-between p-3 rounded-lg hover:bg-gray-100 w-full ${isActive ? 'bg-blue-50 text-blue-600' : 'text-gray-600'
                }`}
        >
    ...
    const SubMenuItem = ({ text, icon: Icon, isActive, onClick }) => (
        <button
            onClick={onClick}
            className={`flex items-center space-x-3 p-2 pl-11 rounded-lg hover:bg-gray-100 w-full ${isActive ? 'bg-blue-50 text-blue-600' : 'text-gray-600'
                }`}
        >
